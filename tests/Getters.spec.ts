...
import { Test, Test_getterMapping } from "../wrappers/Getters";
...
describe("getters", () => {
    ...
    beforeEach(async () => {
        ...
        contract = blockchain.openContract(await Test.fromInit({$$type: 'SetIdAndData', id: 0n, data: beginCell().endCell()}));
    ...
    it("should implement getters correctly", async () => {
        ...
        expect(
            (await contract.getMessageAsInput3({
                $$type: 'SetIdAndData',
                id: 0n,
                data: beginCell().endCell()
            })).toString()
        ).toBe(contract.address.toString())
    });
});